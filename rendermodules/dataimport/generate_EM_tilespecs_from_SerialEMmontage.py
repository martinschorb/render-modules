...
class GenerateSBEMImageTileSpecs(StackOutputModule):
    ...
    def ts_from_SerialEMtile(self,line,pxs,rotation):
   ...
   # 2) The translation matrix to position the object in space (lower left corner)
    ...
    def ts_from_sbemimage (self,idocfile,mapsection=0,correct_gradient=True):

        os.chdir(os.path.dirname(idocfile))
        ...
        # mipmap_args = []
        # tilespecpaths = []
        logfile = os.path.join(imgdir,'conv_log','SerialEM_convert'+log_name+'.log')
        ...
        merged = em.mergemap('',mapfile = imfile,blendmont=False,mapsection=mapsection)
